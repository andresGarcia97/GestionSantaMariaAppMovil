...
const reservasStorage = 'reservas';
...
export class ReservasService {
  ...
  public async getReservas(reserva: Reserva) {
    try {
      ...
      const reservas = await this.http.post<Reserva[]>(OBTENER_RESERVAS_FUTURAS, reserva, options).toPromise();
  ...
  private async guardarReservas(reservas: Reserva[]) {
    await this.storage.set(reservasStorage, JSON.stringify(reservas));
  }

  private async cargarReservas() {
    this.reservasString = await this.storage.get(reservasStorage) || null;
